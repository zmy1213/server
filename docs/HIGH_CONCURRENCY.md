...
```text
QPS：每秒能成功处理多少请求
并发连接数：同时有多少 TCP 连接在请求
延迟：每个请求从发出到收到完整响应用了多久
错误数：连接失败、超时、状态码不符合预期的数量
```
...
```text
创建 N 个 asyncio worker
        ↓
每个 worker 打开一个 TCP 连接
        ↓
在这个连接上循环发送 HTTP 请求
        ↓
读取并解析 HTTP 响应
...
| 功能 | 代码位置 | 说明 |
| --- | --- | --- |
| HTTP 压测脚本 | [`tools/bench_http.py`](../tools/bench_http.py) | 统计 QPS、并发、延迟、错误数 |
| 压测说明文档 | [`docs/BENCHMARK.md`](BENCHMARK.md) | 解释参数和输出指标 |

小白理解：

```text
功能测试：证明代码对不对
压测：证明代码在压力下还能不能稳定、快不快
```
